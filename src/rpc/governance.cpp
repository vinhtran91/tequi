...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 TEQUICHAIN irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on tequichaincentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
        ...
        "Vote on a governance object by smartnode configured in tequichain.conf\n"
        "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.pushKV("tequichain.conf", statusObj);
...
#endif // ENABLE_WALLET
        "  vote-conf          - Vote on a governance object by smartnode configured in tequichain.conf\n"
#ifdef ENABLE_WALLET
...
static const CRPCCommand commands[] =
    {
        ...
        //  --------------------- ------------------------  -----------------------  ----------
        /* Tequichain features */
        {"tequichain", "getgovernanceinfo", &getgovernanceinfo, {}},
        {"tequichain", "getsuperblockbudget", &getsuperblockbudget, {"index"}},
        {"tequichain", "gobject", &gobject, {}},
        {"tequichain", "voteraw", &voteraw, {"tx_hash", "tx_index", "gov_hash", "signal", "outcome", "time", "sig"}},
