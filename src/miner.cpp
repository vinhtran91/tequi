...
void static TequichainMiner(const CChainParams& chainparams)
{
    LogPrintf("TequichainMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tequichain-miner");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("TequichainMiner -- Wallet not available\n");
    }
#endif
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get()) {
                LogPrintf("TequichainMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("TequichainMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        LogPrintf("TequichainMiner -- terminated\n");
        throw;
    } catch (const std::runtime_error& e) {
        LogPrintf("TequichainMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateTequichains(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++) {
        minerThreads->create_thread(boost::bind(&TequichainMiner, boost::cref(chainparams)));
    }
    return (numCores);
}
