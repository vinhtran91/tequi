...
const char* const BITCOIN_CONF_FILENAME = "tequichain.conf";
const char* const BITCOIN_PID_FILENAME = "tequichaind.pid";
...
public:
    ...
    static inline std::pair<bool, std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "tequichaind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TequichainCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TequichainCore
    // Mac: ~/Library/Application Support/TequichainCore
    // Unix: ~/.tequichaincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TequichainCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TequichainCore";
#else
    // Unix
    return pathRet / ".tequichaincore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty tequichain.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Tequichain Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Tequichain Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Tequichain Core developers";
    }
