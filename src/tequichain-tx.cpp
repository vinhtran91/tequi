...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help")) {
        ...
        std::string strUsage = strprintf("%s tequichain-tx utility version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
                               "Usage:\n"
                               "  tequichain-tx [options] <hex-tx> [commands]  Update hex-encoded tequichain transaction\n" +
                               "  tequichain-tx [options] -create [commands]   Create hex-encoded tequichain transaction\n" +
                               "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded tequichain transaction
