...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/TequichainParams"
else
    PARAMS_DIR="$HOME/.tequichain-params"
fi
...
DOWNLOAD_URL="https://github.com/Tequichain-Network/TequichainCore/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with TEQ_DISABLE_SOMETHING=1
TEQ_DISABLE_WGET="${TEQ_DISABLE_WGET:-}"
#TEQ_DISABLE_IPFS="${TEQ_DISABLE_IPFS:-}"
TEQ_DISABLE_CURL="${TEQ_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$TEQ_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$TEQ_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$TEQ_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Tequichain parameters!
...
This script will fetch the Tequichain SNARK parameters and verify their
...
EOF
    ...
    if ! [ -d "$PARAMS_DIR" ]; then
        ...
        cat >>"$README_PATH" <<EOF
This directory stores common Tequichain SNARK parameters. Note that it is
