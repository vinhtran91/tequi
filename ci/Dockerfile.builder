...
# tequichain_hash
RUN git clone https://github.com/Tequichain-Network/TequichainCore_hash
RUN cd tequichain_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} tequichain
RUN useradd -u ${USER_ID} -g tequichain -s /bin/bash -m -d /tequichain tequichain
...
RUN mkdir /tequichain-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /tequichain-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /tequichain-src
